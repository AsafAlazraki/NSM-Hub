...
function EditMotorPageContent() {
    ...
    const motorFactoryCategories = useMemo(() => {
        const motorOptionCategoryIds = new Set(motorFactoryOptions.map(o => o.categoryId).filter(Boolean));
                                        ...
                                        <div className="space-y-4">
                                            {motorFactoryCategories.map(cat => (
                                                <div key={cat.id} className="space-y-2">
                                                    <h4 className="font-semibold flex items-center gap-2">{cat.name} <Badge className={cn("text-xs", cat.tag && tagColors[cat.tag])}>{cat.tag}</Badge></h4>
