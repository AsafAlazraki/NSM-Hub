...
export default function PrintQuotePage() {
  ...
  const isMotorSectionVisible = quote.motors && Array.isArray(quote.motors) && quote.motors.length > 0;
    ...
    <>
      ...
      <div className="no-print fixed top-4 left-1/2 -translate-x-1/2 flex justify-center gap-4 mb-8 z-50">
            <Button variant="outline" asChild>
              <Link href={`/service-hub/estimates/${quote.id}`}><ArrowLeft/> Back to Estimate View</Link>
          </Button>
                          ...
                          {isMotorSectionVisible && (
                              <div className="mb-2">
                                  {quote.motors?.map((motor, index) => {
                                    ...
                                    return <p key={motor.id}><strong>Motor {quote.motors.length > 1 ? index + 1 : ''}:</strong> {[motor.make, motor.model].filter(Boolean).join(' ')}</p>
                                })}
                              </div>
                          )}
