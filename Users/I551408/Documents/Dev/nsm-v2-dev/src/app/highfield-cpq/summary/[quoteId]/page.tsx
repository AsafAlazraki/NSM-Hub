...
function CPQSummaryPageContent() {
    ...
    const {
        ...
        const selectedHull = { name: `${bmtQuote.selectedMaterial} ${bmtQuote.modelName}`, price: hullPrice, includesGst: hullPricing?.includesGst || false, imageUrl: selectedColor?.images?.[0]?.url };
        ...
        addToSubtotal(selectedHull?.price, selectedHull?.includesGst);
        addToSubtotal(selectedConsole?.price, true);
        addToSubtotal(selectedMotor?.sellPrice, true);
        ...
        if (bmtQuote.motorIncludesPreDelivery) subtotal += 0;
        if (bmtQuote.riggingIncludesInstallation) subtotal += 0;
        if (bmtQuote.trailerIncludesPreDelivery) subtotal += 0;
        if (bmtQuote.trailerIncludesRegistration) subtotal += 0;
    ...
    const handleFinalize = () => {
        if (!bmtQuote) return;
        const updatedQuote: BMTQuote = { ...bmtQuote, status: 'Draft' };
        setBmtQuote(updatedQuote);
        saveBmtQuote(updatedQuote);
            ...
            <Header>
                <div className="flex items-center gap-2">
                    <Button asChild variant="outline">
                        <Link href={`/highfield-cpq/configure/${brandId}/${selectedModelId}/dealer-fit?quoteId=${quoteId}`}>
                            <ArrowLeft /> Back
                        </Link>
                    </Button>
                </div>
             ...
             <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
                <div className="mb-8">
                    <ConfigurationStepper brandId={brandId} modelId={selectedModelId} quoteId={quoteId} currentStep="summary" />
                </div>
...
export default function CPQSummaryPage() {
    const { user, loading: authLoading } = useAuth();
    const router = useRouter();
    const auth = getAuth();
