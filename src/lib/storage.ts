...
export const saveDealerFitBrand = async (brandData: Partial<DealerFitBrand>) => {
  ...
  let finalData: Omit<DealerFitBrand, 'id' | 'logoFile'> = {
    name: brandData.name!,
    logo: brandData.logo,
  };

  if (brandData.logoFile) {
    const logoUrl = await uploadFileToStorage(brandData.logoFile, `dealer-fit-brand-logos/${docId}`);
    finalData.logo = logoUrl;
  }
...
export const deleteBmtQuote = async (quoteId: string): Promise<void> => {
    try {
        const quoteDocRef = doc(db, BMT_QUOTES_COLLECTION, quoteId);
        await updateDoc(quoteDocRef, { status: 'Archived' });
    } catch (e) {
        console.error(`Failed to archive BMT quote ${quoteId}`, e);
        throw e;
    }
};
...
export const saveCatalogueMotor = async (motorData: Partial<CatalogueMotor>) => {
    ...
    // Create a clean object for Firestore, excluding temporary/client-side fields.
    const { id, imageFile, propellers, ...dataToSave } = motorData;
  
    if (motorData.imageFile) {
      const imageUrl = await uploadFileToStorage(motorData.imageFile, `motor-images/${docId}`);
      dataToSave.imageUrl = imageUrl;
    }
  
    await setDoc(docRef, { ...dataToSave, id: docId }, { merge: true });
    
    // Return a complete object, including the ID, for UI updates.
...
export const saveCatalogueTrailer = async (trailerData: Partial<CatalogueTrailer>) => {
    ...
    let finalData: Omit<CatalogueTrailer, 'id' | 'imageFile'> = {
        name: trailerData.name!,
        price: trailerData.price || 0,
        specifications: trailerData.specifications || [],
        imageUrl: trailerData.imageUrl,
        compatibleFactoryOptionIds: trailerData.compatibleFactoryOptionIds || [],
    };
