...
import { Ship, User, MoreVertical, Edit, Trash2 } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export const BmtQuoteCard = ({ quote, onDelete }: { quote: BMTQuote, onDelete: (quoteId: string) => void }) => {
    ...
    const { pricing, hullIncludesRegistration } = quote;
    ...
    const handleStatusChange = async (newStatus: BMTQuoteStatus) => {
        try {
            ...
            toast({
             ...
             // We may need to refresh the list here if sorting/filtering is affected
        } catch (error) {
             toast({
                variant: 'destructive',
                title: 'Update Failed',
        ...
        <Card className="flex flex-col h-full hover:shadow-lg transition-shadow duration-300">
             <CardHeader>
                <div className="flex justify-between items-start">
                    <CardTitle className="font-headline text-lg leading-tight">{quote.brandName} {quote.selectedModelId}</CardTitle>
                    <AlertDialog>
                        <DropdownMenu>
