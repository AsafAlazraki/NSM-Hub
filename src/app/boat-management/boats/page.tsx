...
function BoatsPageContent() {
    ...
    const handleSaveBrand = async (name: string) => {
        try {
            await saveBoatBrand({ id: editingBrand?.id, name });
            toast({ title: "Brand Saved", description: `"${name}" has been saved.` });
            fetchData();
        } catch (error) {
        ...
        <div className="flex flex-col h-screen">
            <Header>
                 <Button asChild variant="outline">
                    <Link href="/boat-management/dashboard"><ArrowLeft /> Back to Dashboard</Link>
                </Button>
            </Header>
            <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
                ...
                {/* Dialogs */}
                <EntryFormDialog isOpen={isBrandFormOpen} setIsOpen={setIsBrandFormOpen} onSave={handleSaveBrand} entryType="Brand" initialName={editingBrand?.name} />
