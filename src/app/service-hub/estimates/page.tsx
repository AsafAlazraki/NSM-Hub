...
function ServiceEstimatesDashboardPageContent() {
  ...
  const statusCounts = useMemo(() => {
    ...
    quotes.forEach(quote => { (quote.history || []).forEach(id => historyIds.add(id)); });
    
    const activeQuotes = quotes.filter(q => !historyIds.has(q.id) && q.status !== 'Deleted');
  ...
  const filteredQuotes = useMemo(() => {
    let sourceQuotes;
    if (showArchived) {
        sourceQuotes = archivedQuotes;
    } else {
        const historyIds = new Set<string>();
        quotes.forEach(quote => {
          (quote.history || []).forEach(id => historyIds.add(id));
        });
        sourceQuotes = quotes.filter(q => !historyIds.has(q.id) && q.status !== 'Deleted');
    }
