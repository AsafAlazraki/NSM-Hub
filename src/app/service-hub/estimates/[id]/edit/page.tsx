...
import { getQuoteById, createNewVersion, getStaticLogo, getUserProfile } from "@/lib/storage";
...
function EditQuotePageContent() {
        ...
        const handleVersioning = async () => {
            if (user && id && !versioningStarted.current) {
                ...
                try {
                    ...
                    router.replace(`/service-hub/estimates/${newVersionId}/edit`, { scroll: false });
                    ...
                    router.push(`/service-hub/estimates/${id}`);
                }
            }
        };
             ...
             <div className="flex flex-col h-full">
                <Header>
                    <Button variant="ghost" asChild>
                        <Link href={id ? `/service-hub/estimates/${id.split('-v')[0]}` : '/service-hub/estimates'}>Cancel</Link>
                    </Button>
                </Header>
    ...
    return (
        <div className="flex flex-col h-full">
            <Header>
                <Button variant="ghost" asChild>
                    <Link href={`/service-hub/estimates/${id.split('-v')[0]}`}>Cancel</Link>
                </Button>
            </Header>
