...
function CustomerHubPageContent() {
    ...
    const sortedAndFilteredCustomers = useMemo(() => {
        ...
        const customerDataMap = quotes.reduce((acc, q) => {
            if (!q || !q.customer || !q.customer.name) return acc;
            const key = getCustomerId(q.customer);
            ...
            if ('operations' in q && q.boat && q.boat.registration) {
                assetIdentifiers.add(q.boat.registration);
            } else if ('brandName' in q && q.brandName && q.selectedModelId) {
                assetIdentifiers.add(`${q.brandName} ${q.selectedModelId}`);
            }
            ...
            acc.set(key, {
                ...
                lastActivity: Math.max(existing.lastActivity, new Date(q.createdAt).getTime()),
                assetCount: existing.assetCount,
                quotes: [...existing.quotes, q]
            });
            return acc;
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {sortedAndFilteredCustomers.map(customer => {
                                ...
                                customerQuotes.forEach((q: Quote | BMTQuote) => {
                                    if (!q) return;
                                    let assetIdentifier: string | undefined;

                                    if ('operations' in q && q.boat?.registration) {
                                        assetIdentifier = `${q.boat.make || ''} ${q.boat.model || ''} (${q.boat.registration})`.trim();
                                    } else if ('brandName' in q && q.brandName && q.selectedModelId) {
                                        assetIdentifier = `${q.brandName} ${q.selectedModelId}`;
                                    }
