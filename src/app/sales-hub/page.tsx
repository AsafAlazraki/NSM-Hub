...
function SalesPipelineChart({ quotes }: { quotes: BMTQuote[] }) {
    const data = [
        ...
        { name: 'Sent', count: quotes.filter(q => q.status === 'Sent to Customer').length, fill: 'hsl(var(--chart-2))' },
        { name: 'Approved', count: quotes.filter(q => q.status === 'Approved').length, fill: 'hsl(var(--chart-3))' },
        { name: 'Not Approved', count: quotes.filter(q => q.status === 'Not Approved').length, fill: 'hsl(var(--chart-4))' },
        { name: 'Cancelled', count: quotes.filter(q => q.status === 'Cancelled').length, fill: 'hsl(var(--chart-5))' },
    ];
